...
// Cap transcript size at ingest. Snapshots come from the CLI (or a
// mirrored snapshot file on disk), which is trusted in principle but
...
export async function syncSnapshots(repo: { id: string; path: string; provider: string; orgId: string }) {
...
async function syncGitHubRepo(repo: { id: string; path: string; orgId: string }) {
  ...
  // 1. Check for .entire/ snapshots (real session data from Entire CLI)
  const entireFiles = await fetchEntireSnapshots(owner, repoName);
  ...
  for (const file of files) {
    ...
    try {
      ...
      const existing = await prisma.commit.findFirst({
        where: { repoId: repo.id, sha: snapshot.commitSha },
      });
      ...
      await prisma.commit.create({
        data: {
          ...
          aiToolDetected: snapshot.model,
          aiDetectionMethod: 'session',
          committedAt: new Date(snapshot.committedAt),
          session: {
            create: {
              model: snapshot.model,
              prompt: snapshot.prompt,
              transcript: safeTranscriptJson(snapshot.transcript),
              filesChanged: JSON.stringify(snapshot.filesChanged),
              tokensUsed: snapshot.tokensUsed,
              toolCalls: snapshot.toolCalls,
              durationMs: snapshot.durationMs,
              linesAdded: snapshot.linesAdded ?? 0,
              linesRemoved: snapshot.linesRemoved ?? 0,
      ...
      console.error(`Failed to process snapshot ${file.name}:`, err);
    }
  }
...
async function syncLocalRepo(repo: { id: string; path: string }) {
  ...
  // snapshot ingest or the webhook flow. In that environment, "sync"
  // means: recount what's in the DB and let the outer syncSnapshots
  ...
  // snapshots. So we only short-circuit when the path doesn't exist.
...
async function syncFromEntireDir(repo: { id: string; path: string }, entireDir: string) {
  ...
  for (const file of files) {
    try {
      ...
      // Skip malformed snapshots, but surface *why* — the old silent
      ...
      console.error('[snapshot] failed to import file', {
        repoId: repo.id,
        file,
