...
export default function SharedSession() {
  ...
  useEffect(() => {
    ...
    getSharedSession(slug)
      .then((data: SharedSessionData) => {
        setSession(data);
        return getAnnotations(data.id);
      })
      .then((anns: SessionAnnotation[]) => setAnnotations(anns))
                  ...
                  <div key={pc.promptIndex} className="rounded-lg border border-gray-800 bg-gray-900/50 overflow-hidden">
                    ...
                    {expandedPrompts.has(pc.promptIndex) && (
                      <div className="border-t border-gray-800/50 px-4 py-3 space-y-3">
                        ...
                        {/* Annotations (read-only on shared view) */}
                        {turnAnnotations.length > 0 && (
...
function AnnotationCallout({ annotation }: { annotation: SessionAnnotation }) {
  return (
    <div className="flex gap-2 rounded-md border border-indigo-500/20 bg-indigo-500/5 px-3 py-2">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-gray-500 mt-1">
          {annotation.authorName} &middot; {new Date(annotation.createdAt).toLocaleString()}
        </p>
      </div>
    </div>
  );
