...
export async function dbImportCommand(opts?: { format?: string; file?: string }): Promise<void> {
  ...
  // If snapshotRepo is configured, fetch origin-sessions from external repo first.
  ...
  if (config?.snapshotRepo) {
    console.log(chalk.gray(`Fetching origin-sessions from snapshot repo...`));
    try {
      git(
        ['fetch', '--filter=blob:none', config.snapshotRepo, 'origin-sessions:origin-sessions', '--force'],
        { cwd: repoPath, timeoutMs: 30_000 },
      );
    } catch (err: any) {
      console.log(chalk.yellow(`Warning: Could not fetch from snapshot repo: ${err.message}`));
    }
  }
