// Origin's MCP server — the read path that lets an agent ask Origin what it
// already knows instead of re-deriving it. Two halves, deliberately:
//
//   • GIT-side, offline: get_file_context reads refs/notes/origin locally, so
//     the prior agent's prompts, [Origin: …] markers and fragility signal are
...
// releases. Folding it in gives it one version, one signed artifact, and the
// CLI's auth/config/profile handling instead of a second copy of all three.
//
// Started by `origin mcp serve`, which speaks stdio to the agent. IMPORTANT:
// stdout is the JSON-RPC channel — every diagnostic here MUST go to stderr, or
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    ...
    },
    {
      ...
      description: 'Retrieve the intent behind specific files from Origin\'s git-notes history BEFORE you modify unfamiliar code — the prior agent\'s prompts, its [Origin: Decision/Open/Verify] notes (the "why"), which files it read, and a fragility signal (how much of the last change survived on HEAD). Reads local git notes (no network, works offline), newest-first. Token-frugal: by default returns short prompt summaries + a compact `signals` headline per commit (acceptanceRate, fragile flag, counts of decisions/open/verify, filesReadCount) so you can triage cheaply; pass include_detail to pull the full prompts, the full decision/open/verify markers, and the prior agent\'s filesRead list only where it matters.',
      inputSchema: {
        type: 'object' as const,
        properties: {
  ...
  switch (name) {
    ...
    case 'get_file_context': {
      try {
        ...
        const result = getFileContext(repoPath, paths, {
          perPathLimit: (args?.per_path_limit as number) || undefined,
 ...
 *
 * Never throws for a missing/!invalid Origin config: the git-notes half
 * (get_file_context) works offline with no account, and that is precisely the
 * tool an agent reaches for on a fresh clone. Refusing to start would take the
 * offline half down with the online one — the server-backed tools surface their
 * own auth error per call instead.
 */
export async function runMcpServer(): Promise<void> {
  ...
  if (config) {
    ...
    console.error('[origin-mcp] No Origin config — git-notes tools still work offline.');
    console.error('[origin-mcp] For session/stats/policy tools: origin login && origin enable');
  }

  const transport = new StdioServerTransport();
  await server.connect(transport);
  console.error('[origin-mcp] Origin MCP server running (stdio)');
}
